...
## Remove w1118;7837 samples 21-07_30 ONLY - 1-vial climbing and boxplots show outliers - this point falls right on line in 2022 poster
climb2 <- climb2[which(climb2$mitogeno != "w1118_7837"), ]
dim(climb2)

## Remove all w1118 mtDNA samples 21-07_30 to make this set fully factorial
climb2 <- climb2[which(climb2$mito != "w1118"), ]
dim(climb2)
...
fit2f22 <- aov(slope~mito*geno,data = climb2f22)
summary(fit2f22)

fit2f22a <- aov(slope~geno*mito,data = climb2f22)
summary(fit2f22a)

## fit2flm <- lm(slope~mito*geno*day,data = climb2f)
## summary(fit2flm)

## fit1m <- aov(slope~mito*geno*day,data = climb1m) ## climb1m uses csv file based on one (1) ROI in film analyses for males (m).
## summary(fit1m)

fit2m <- aov(slope~mito*geno*day,data = climb2m) ## climb2m uses csv file based on two (2) ROI in film analyses for males (m).
summary(fit2m)

fit2ma <- aov(slope~geno*mito*day,data = climb2m) ## reverse order of mito & geno to compare order of aov type 1 SSQ
summary(fit2ma)

fit2m22 <- aov(slope~mito*geno,data = climb2m22)
summary(fit2m22)

fit2m22a <- aov(slope~geno*mito,data = climb2m22)
summary(fit2m22a)

fit2mlm <- lm(slope~mito*geno*day,data = climb2m) ## lm gives individual terms.
summary(fit2mlm)
Anova(fit2mlm, type="III")

table(climb2f$mito, climb2f$geno, climb2f$day)
## Shows females not balanced : no w1118 and
...
options(contrasts = c("contr.sum", "contr.poly"))
fit2mmod3 = lm(slope ~ mito * geno * day, data = climb2m, 
               contrasts = list(mito = "contr.sum", geno = "contr.sum", day = "contr.sum"))
Anova(fit2mmod3, type="III")

print(fit2mmod3_result)

## google for "code for 3-way anova type III sum of squares in R"
## Assuming your data is in a data frame called 'data'
## and your dependent variable is 'DV', and independent variables are 'A', 'B', and 'C'
## Fit the model with interaction terms and specified contrasts
## model <- lm(DV ~ A * B * C, data = data, contrasts = list(A = "contr.sum", B = "contr.sum", C = "contr.sum"))
...
mean_climb_1491set.df <- data.frame(mean_climb)
write.csv(mean_climb_1491set.df, '/Users/davidrand/Brown Dropbox/David Rand/Manuscripts/Mito Deficiency Screen/Mito Deficiency Climbing/2021_07_30_results/mean_climb1491set.csv')
...
write.csv(cv_by_sex, '/Users/davidrand/Brown Dropbox/David Rand/Manuscripts/Mito Deficiency Screen/Mito Deficiency Climbing/2021_07_30_results/cv_by_sex_1491set.csv')
