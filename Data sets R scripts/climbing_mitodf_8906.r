...
climb2 <- climb2[ grep("Cy", climb2$chromo, invert = TRUE) , ]
dim(climb2)
...
climb2m2day %>% 
  ...
  ylim(0.5,4.0) +
  labs(
    title = "Male Climbing of Mito-Deficiency Genotypes at Ages 7 or 24 Days",
...
climb2f2day %>% 
  ...
  #theme_classic() +
  labs(
    title = "Female Climbing of Mito-Deficiency Genotypes at Ages 7 or 24 Days",
...
mean_climb_8906set.df <- data.frame(mean_climb)
write.csv(mean_climb_1491set.df, '/Users/davidrand/Brown Dropbox/David Rand/Manuscripts/Mito Deficiency Screen/Mito Deficiency Climbing/batch8906_9061/mean_climb8906set.csv')
...
write.csv(cv_by_sex, '/Users/davidrand/Brown Dropbox/David Rand/Manuscripts/Mito Deficiency Screen/Mito Deficiency Climbing/batch8906_9061/cv_by_sex8906set.csv')
...
## Leah's genious bootstrap code email 2025-06-20
...
pbootclimb = ggplot(boot_climb, aes(y = boot, x = geno, fill = sex, color=sex)) +
  geom_point(position=position_jitterdodge(jitter.width = 0.4),size=0.4) +
  geom_boxplot(color="black", alpha=0.5, outliers = FALSE)  + 
  facet_wrap(~day) +
  labs(title = "Climbing",
       x = "Genotype,Environment",
...
## CODE FOR CV calculations - requires custom file editing
